...
/*                //disparity happens
    ...
    int kernel(std::vector<std::pair<float, float>> & array, gap largest_gap){
        ...
        int front = a / 2;
        int rear = a - front;
        //create a new array called "window", with half of the kernel size with value 0 on two sides of the array
...
/*        float sum = std::accumulate(window.begin(), window.begin()+kernel_size, 0);
        float window_average = sum / kernel_size;
        temp.push_back(window_average);*/
        float sum;
        float window_average;
        //filling up "temp" with the convolved value
        for(int i = 0; i < largest_gap.gap_size; i++){
            sum = std::accumulate(window.begin()+i, window.begin()+i+kernel_size, 0);
            temp.push_back(sum / kernel_size);
        }
        //assign the elements in temp back to array
        for(int i = 0; i < temp.size(); i++){
            array[i + largest_gap.gap_start_index].second = temp[i];
        }
        int direction_index = 0;
        for(int i = largest_gap.gap_start_index; i < largest_gap.gap_start_index + largest_gap.gap_size; i++){
            if(array[i] > array[direction_index]){
                direction_index = i;
            }
        }
...
public:
    ...
    void scan_callback(const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
        ...
        int direction_index = kernel(processed_range, largest_gap);
