...
//update occupancy grid with dynamic and static obstacles
void RRT::scan_callback(const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
    // The scan callback, update occupancy grid with dynamic and static obstacles here
    // Args:
    //    scan_msg (*LaserScan): pointer to the incoming scan message
    std::vector<float> range_laser = scan_msg->ranges;

    float min_angle = scan_msg->angle_min;
    float angle_inc = scan_msg->angle_increment;
    occumap.data = levine_map_ptr->data;//reinitialize static occumap every time
    Dyna_Map_Update(min_angle, angle_inc, range_laser);//add dynamic obstacle into occumap every time

    map_pub_.publish(occumap);//publish to rviz
}

//find the waypoint that is closest to lookahead wapoint distance as to car
int RRT::getClosestWp(std::vector<double> & wp_x_l, std::vector<double> & wp_y_l){
    ...
    double dist = DBL_MAX;
    double gap = DBL_MAX;
    int closest_wp_index;

    for (int i = 0; i < wp_y_l.size(); i++) {
        dist = pow(pow(wp_x_l[i], 2) + pow(wp_y_l[i], 2), 0.5);
        gap = fabs(lookahead_wp - dist);
        // the closest is set to inf initially
        if (min_gap_wp > gap && wp_x_l[i] >= 0.0) {
            if (occumap.data[rc_2_ind(getRowFromPoint(wp_y_g[i]), getColFromPoint(wp_x_g[i]))] == 0) {
                min_gap_wp = gap;
                closest_wp_index = i;
            }
        }
...
void RRT::pf_callback(const nav_msgs::Odometry::ConstPtr &pose_msg) {
    ...
    // Returns:
    //
    // tree as std::vector of Nodes
    std::vector<Node> tree;
    std::vector<Node> path;

    Node start_Node;//initialize the start node of tree
    start_Node.x = (pose_msg->pose).pose.position.x;//in global frame
    start_Node.y = (pose_msg->pose).pose.position.y;
    start_Node.is_root = true;
    start_Node.cost = 0;
    tree.push_back(start_Node);

    double yaw = getYaw(pose_msg);

    //waypoint coordinate in laser frame
    std::vector<double> wp_x_l(wp_x_g.size(), 0);
    std::vector<double> wp_y_l(wp_y_g.size(), 0);
    ...
    for (int i = 0; i < wp_y_g.size(); i++) {
        map_wypt.point.x = wp_x_g[i];
        map_wypt.point.y = wp_y_g[i];

        lidar_wypt = transformFromMap2Lidar(map_wypt);

        wp_x_l[i] = lidar_wypt.point.x;//delta_x * cos(yaw) + delta_y * sin(yaw);
        wp_y_l[i] = lidar_wypt.point.y;//-delta_x * sin(yaw) + delta_y * cos(yaw);
    }

    //find the waypoint that is closest to lookahead waypoint distance as to car
    int closest_wp_index = getClosestWp( wp_x_l, wp_y_l);

    double goal_x_g = wp_x_g[closest_wp_index];//in global frame
    double goal_y_g = wp_y_g[closest_wp_index];
    ...
    visualise_markers(marker_goal, goal_x_g, goal_y_g, 1);
    ...
    for (int i = 0; i < MAX_ITER; i++) {
        ...
        std::vector<double> sample_point = sample(goal_x_g, goal_y_g);
        ...
        if (collision == true) {//look for next point
            continue;
        } else {
            ...
            tree_line_marker.id++;
            vis_tree_edge_pub.publish(tree_line_marker);
            if (is_goal(new_point, goal_x_g, goal_y_g)) {
    ...
    vis_path_edge_pub.publish(path_line_marker);
    ...
    if (k > 0){visualise_markers(marker_chasing, path[k + 1].x, path[k + 1].y, 1);}
...
//visualize points, including waypoint, current goal waypoint, current path nodes
...
void RRT::Dyna_Map_Update(float min_angle, float angle_inc,  std::vector<float> & range_laser) {
//input: laser range
//output:update dynamic layer map
    int array_size = 0;
    ...
    for (std::vector<float>::iterator it = range_laser.begin() + 180; it != range_laser.end() - 180; ++it) {
        ...
        if (beam < detection_dist)
            ObsInflation(row, col);
    }
}
...
//get yaw angle of car
double RRT::getYaw(const nav_msgs::Odometry::ConstPtr &pose_msg){
    tf::Quaternion q(
...
//inflate the detected obstacles
void RRT::ObsInflation(int row, int col) {
...
std::vector<double> RRT::sample(double &goal_x_g, double goal_y_g) {
    ...
    while (free_indicator == 0) {
        ...
        if (prob_indi % sample_goal_prob_reciprocal == 0) {
            lidar_pt.point.x = goal_x_g;
            lidar_pt.point.y = goal_y_g;
            lidar_pt.point.z = 0;
//            free_indicator = 1;
        } else {
...
bool RRT::is_goal(Node &latest_added_node, double &goal_x_g, double &goal_y_g) {
    // This method checks if the latest node added to the tree is close
    // enough (defined by goal_threshold) to the goal so we can terminate
    // the search and find a path
    // Args:
    //   latest_added_node (Node): latest addition to the tree
    //   goal_x_g (double): x coordinate of the current goal
    //   goal_y_g (double): y coordinate of the current goal
    // Returns:
    //   close_enough (bool): true if node close enough to the goal

    bool close_enough = false;
    float dist = sqrt(pow(latest_added_node.x - goal_x_g, 2) + pow(latest_added_node.y - goal_y_g, 2));
