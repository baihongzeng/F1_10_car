...
private:
    ...
    //waypoint stuff
    std::vector<double> wp_x_g;//the x-coordinate of waypoints
    std::vector<double> wp_y_g;//the y-coordinate of waypoints
    std::string temp;
    ...
    ros::Publisher vis_tree_edge_pub;
    ros::Publisher vis_path_edge_pub;
    ...
    //scan_callback
    void Dyna_Map_Update(float min_angle, float angle_inc,  std::vector<float> & range_laser);
    void ObsInflation(int row, int col);

    //pf_callback
    double getYaw(const nav_msgs::Odometry::ConstPtr &pose_msg);
    int getClosestWp(std::vector<double> & wp_x_l, std::vector<double> & wp_y_l);
