  ...
  public:
    ...
    void handleLaserScan(const sensor_msgs::LaserScan::ConstPtr& msg) {
      ...
      if(prev_points.empty()){
        ROS_INFO("First Scan");
        prev_points = points; //'points' is the points in the current frame
        return;
      }
    ...
    void readScan(const sensor_msgs::LaserScan::ConstPtr& msg) {
      ...
      for (int i = 0; i < ranges.size(); ++i) {
        ...
        if (range > RANGE_LIMIT) { //get rid of the range > 10
          continue;
        }
        if (!isnan(range) && range >= range_min && range <= range_max) {
          points.push_back(Point(range, angle_min + angle_increment * i));//range and corresponding angle
        }
      }
    ...
    void publishPos() {
     ...
     msg.header.frame_id = "map";//used to be "laser"
